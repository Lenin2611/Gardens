    ...
    public class PedidoRepository : GenericRepositoryInt<Pedido>, IPedido
    {
        ...
        public async Task<IEnumerable<Pedido>> GetPedidosRechazados2009Async() // 2611
        {
            ...
            (
                ...
                WHERE e.Nombre = 'rechazado' AND YEAR(p.FechaPedido) = '2009'"
            ).ToListAsync();
            return result;
        }

        public async Task<IEnumerable<Pedido>> GetPedidosEnero() // 2611
        {
            var result = await _context.Pedidos.FromSqlRaw
            (
                @"SELECT p.*
                FROM Pedido p
                WHERE MONTH(p.FechaEntrega) = '1' AND (p.FechaEntrega != '1900-01-01')"
            ).ToListAsync();
            return result;
        }
    }
